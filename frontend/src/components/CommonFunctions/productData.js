export const productsData = [
  {
    id: "1",
    name: "Samsung Galaxy S21",
    img: "https://images.news18.com/ibnlive/uploads/2021/02/1613657806_samsung-galaxy-s21-1-2.jpg?impolicy=website&width=510&height=356",
    category: "Electronics",
    price: 799.99,
    rating: 4.5,
    description:
      "The Samsung Galaxy S21 is a high-end smartphone with a stunning display and powerful performance.",
    comments: [
      "Great phone! I love the camera quality.",
      "Fast delivery and excellent customer service.",
      "The battery life could be better.",
    ],
  },
  {
    id: "28",
    name: "Nike Air Max 270",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTCNyXlpoZxDY9u0z-79xBr2MgP3hlgFLY2xA&usqp=CAU",
    category: "Clothing",
    price: 129.99,
    rating: 4.2,
    description:
      "Nike Air Max 270 sneakers provide both comfort and style for everyday wear.",
    comments: [
      "Super comfortable shoes!",
      "The design is really cool.",
      "Sizing runs a bit small.",
    ],
  },
  {
    id: "2",
    name: "Apple Watch Series 7",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRul0SmJ4DldoPPENbhFCJfiv-7j3a-coItXQ&usqp=CAU",
    category: "Electronics",
    price: 349.99,
    rating: 4.8,
    description:
      "The Apple Watch Series 7 offers advanced health tracking and a beautiful Retina display.",
    comments: [
      "I can't live without my Apple Watch!",
      "Great for fitness tracking and notifications.",
      "Expensive but worth it.",
    ],
  },
  {
    id: "3",
    name: "Modern Coffee Table",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSaGfZizALbSScnNbfWAFaXeyT55-mR3jm1ClBOCtD5tO61N2COofCWouwTYvv28vlMOYs&usqp=CAU",
    category: "Furniture",
    price: 199.99,
    rating: 4.0,
    description:
      "This sleek and stylish coffee table will enhance the look of your living room.",
    comments: [
      "Looks great in my living room.",
      "Easy to assemble.",
      "The quality could be better for the price.",
    ],
  },
  {
    id: "4",
    name: "Kitchen Aid Stand Mixer",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ3eijpi6lgV6G96e8K60KmrgGx_pRWAJNXJA&usqp=CAU",
    category: "Appliances",
    price: 299.99,
    rating: 4.7,
    description:
      "The KitchenAid Stand Mixer is a must-have for every home baker and chef.",
    comments: [
      "I use it all the time for baking!",
      "High-quality construction.",
      "A bit heavy to move around.",
    ],
  },
  {
    id: "5",
    name: "Adidas Originals Track Jacket",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTJ0DaQdypaReniZ0J7jUKCDZy4jWiQtz2MlK6Npi47fe7nBw71XqwBZnyxUQtS0V4F1rE&usqp=CAU",
    category: "Clothing",
    price: 79.99,
    rating: 4.4,
    description:
      "Stay stylish and comfortable with this classic Adidas track jacket.",
    comments: [
      "Perfect for workouts and casual wear.",
      "Fits well and looks great.",
      "The zipper quality could be improved.",
    ],
  },
  {
    id: "6",
    name: "Sony 55-Inch 4K Smart TV",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSj6wQvWS7glPvrWp_H_IpUKUrvfLz159mhznJ_npZLUZ1da4xzksZiYmdj956f3ahaCWY&usqp=CAU",
    category: "Electronics",
    price: 699.99,
    rating: 4.6,
    description:
      "Experience stunning visuals and smart features with this Sony 4K TV.",
    comments: [
      "The picture quality is amazing!",
      "Easy setup and user-friendly interface.",
      "A bit expensive, but worth the investment.",
    ],
  },
  {
    id: "7",
    name: "Dining Room Table and Chairs Set",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQcVWPBUtsrIfJHiRgNNruRY_UnPxrF46hRkg&usqp=CAU",
    category: "Furniture",
    price: 499.99,
    rating: 4.3,
    description:
      "Upgrade your dining area with this elegant table and chairs set.",
    comments: [
      "Looks classy and well-made.",
      "Assembly was a bit challenging.",
      "Great value for the price.",
    ],
  },
  {
    id: "8",
    name: "Samsung 55-Inch QLED 4K TV",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRCYgmd2flC5fgrU7fGxSyYw0Zf9WTdwTQy8A&usqp=CAU",
    category: "Electronics",
    price: 899.99,
    rating: 4.7,
    description:
      "Immerse yourself in a world of vibrant colors and exceptional clarity with this Samsung QLED TV.",
    comments: [
      "Absolutely stunning TV!",
      "Browsing and streaming are a breeze.",
      "A bit pricey, but worth every penny.",
    ],
  },
  {
    id: "9",
    name: "Whirlpool French Door Refrigerator",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSFSUZf3D-PkO3cdhEAMjnKPmZwnrZOsfeH9A&usqp=CAU",
    category: "Appliances",
    price: 1499.99,
    rating: 4.5,
    description:
      "Keep your food fresh and organized with this spacious Whirlpool refrigerator.",
    comments: [
      "Plenty of storage space and features.",
      "Energy-efficient and quiet operation.",
      "Expensive, but it's a top-notch fridge.",
    ],
  },
  {
    id: "10",
    name: "Levi's 501 Original Fit Jeans",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSa9Pzcsa63lJ3GwwsGY7osf57WifqCodBC44lLbz2KXLWpvRhR91vfunW9jAplozpJ3As&usqp=CAU",
    category: "Clothing",
    price: 59.99,
    rating: 4.2,
    description:
      "Classic Levi's 501 jeans for a timeless look and comfortable fit.",
    comments: [
      "My go-to jeans for everyday wear.",
      "Durable and well-made.",
      "Sizing can be a bit inconsistent.",
    ],
  },
  {
    id: "11",
    name: "Dyson V11 Cordless Vacuum Cleaner",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRnastAzFT4-dxHIrJ2NaqXS-Wem7JYBMA1VQ&usqp=CAU",
    category: "Appliances",
    price: 399.99,
    rating: 4.8,
    description:
      "Effortlessly clean your home with the powerful Dyson V11 cordless vacuum cleaner.",
    comments: [
      "Amazing suction power!",
      "Convenient and easy to use.",
      "A bit on the expensive side, but worth it.",
    ],
  },
  {
    id: "12",
    name: "Oversized Sectional Sofa",
    img: "https://images.thdstatic.com/productImages/b8b8dee7-8b52-46a9-ad5f-5198c9051a14/svn/dark-gray-magic-home-sectional-sofas-cs-gt000262aae-64_600.jpg",
    category: "Furniture",
    price: 899.99,
    rating: 4.6,
    description:
      "Create a cozy and spacious seating area with this oversized sectional sofa.",
    comments: [
      "So comfortable and roomy.",
      "Assembly was straightforward.",
      "Great value for the size and quality.",
    ],
  },
  {
    id: "13",
    name: "Canon EOS Rebel T7i DSLR Camera",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTvwlHAeKhi0Nwwn_79ZZIjD9o-1P4n_5ODSg&usqp=CAU",
    category: "Electronics",
    price: 699.99,
    rating: 4.4,
    description:
      "Capture high-quality photos and videos with the Canon EOS Rebel T7i DSLR camera.",
    comments: [
      "Excellent camera for beginners and enthusiasts.",
      "Versatile and easy to use.",
      "Consider investing in additional lenses.",
    ],
  },
  {
    id: "14",
    name: "Calvin Klein Women's Crossbody Bag",
    img: "https://media.darveys.com/catalog/product/3/0/30-lot7185_1.jpg",
    category: "Clothing",
    price: 69.99,
    rating: 4.1,
    description:
      "Add a touch of style to your outfit with this Calvin Klein crossbody bag.",
    comments: [
      "Looks elegant and holds essentials.",
      "Perfect for daily use.",
      "The strap quality could be improved.",
    ],
  },
  {
    id: "15",
    name: "LG 65-Inch OLED 4K TV",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRCUrqs6ZGYRXb43i59PmTsFa5NXyt5_28RFg&usqp=CAU",
    category: "Electronics",
    price: 1599.99,
    rating: 4.9,
    description:
      "Experience stunning OLED technology with this LG 65-inch 4K TV.",
    comments: [
      "The best TV I've ever owned!",
      "Incredible picture quality and thin design.",
      "Pricey, but worth every cent.",
    ],
  },
  {
    id: "16",
    name: "Bosch Front-Load Washing Machine",
    img: "https://i.gadgets360cdn.com/products/front-load-washing-machine-wak24168in-large-13652-10556-1584363573.jpeg?downsize=*:360",
    category: "Appliances",
    price: 899.99,
    rating: 4.6,
    description:
      "Efficiently clean your laundry with this Bosch front-load washing machine.",
    comments: [
      "Cleans clothes thoroughly and quietly.",
      "Great energy efficiency.",
      "A bit expensive, but it's a quality machine.",
    ],
  },
  {
    id: "17",
    name: "Puma Men's Running Shoes",
    img: "https://www.jiomart.com/images/product/original/rvsgkfvbcr/puma-men-s-radcliff-puma-black-sports-running-shoe-product-images-rvsgkfvbcr-0-202210212017.jpg?im=Resize=(500,630)",
    category: "Clothing",
    price: 89.99,
    rating: 4.3,
    description:
      "Stay active with these comfortable and stylish Puma running shoes.",
    comments: [
      "Perfect for running and casual wear.",
      "Good cushioning and support.",
      "Sizing runs a bit small.",
    ],
  },
  {
    id: "18",
    name: "Xbox Series X Gaming Console",
    img: "https://5.imimg.com/data5/SELLER/Default/2022/9/YT/AK/PW/159006294/microsoft-xbox-series-x-1tb-video-game-console-black.jpg",
    category: "Electronics",
    price: 499.99,
    rating: 4.7,
    description: "Enjoy next-gen gaming with the Xbox Series X gaming console.",
    comments: [
      "Amazing graphics and performance.",
      "Great selection of games.",
      "Hard to find in stock, but worth the wait.",
    ],
  },
  {
    id: "19",
    name: "Whirlpool Dishwasher",
    img: "https://5.imimg.com/data5/CA/IL/MY-29256728/whirlpool-wfo3o33-dlx-in-49-kg-powerclean-pro-500x500.jpg",
    category: "Appliances",
    price: 599.99,
    rating: 4.5,
    description: "Make dishwashing effortless with this Whirlpool dishwasher.",
    comments: [
      "Cleans dishes thoroughly and quietly.",
      "Easy to use and energy-efficient.",
      "A bit pricey for a dishwasher.",
    ],
  },
  {
    id: "20",
    name: "Adidas Ultraboost Running Shoes",
    img: "https://assets.adidas.com/images/w_383,h_383,f_auto,q_auto,fl_lossy,c_fill,g_auto/c9045494c79b4daca8ad306c22565cdf_9366/ultraboost-light-23-shoes.jpg",
    category: "Clothing",
    price: 169.99,
    rating: 4.6,
    description:
      "Experience exceptional comfort and performance with Adidas Ultraboost running shoes.",
    comments: [
      "Best running shoes I've ever had!",
      "Excellent cushioning and support.",
      "Sizing is accurate.",
    ],
  },
  {
    id: "21",
    name: "Sony Noise-Canceling Headphones",
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS4JrWoSh92gMrl5CbLd_-9cw6Ty-T-VkiWYQ&usqp=CAU",
    category: "Electronics",
    price: 249.99,
    rating: 4.8,
    description:
      "Immerse yourself in your music with these Sony noise-canceling headphones.",
    comments: [
      "Outstanding noise cancellation!",
      "Excellent sound quality.",
      "A bit expensive, but worth it for the peace and sound quality.",
    ],
  },
  {
    id: "22",
    name: "IKEA Hemnes Dresser",
    img: "https://www.vivandtimhome.com/wp-content/uploads/2021/07/ikea-hemnes-dresser-4.jpg",
    category: "Furniture",
    price: 249.99,
    rating: 4.4,
    description:
      "Organize your clothes and accessories with the stylish IKEA Hemnes dresser.",
    comments: [
      "Looks great in my bedroom.",
      "Sturdy and functional design.",
      "Assembly was a bit time-consuming.",
    ],
  },
  {
    id: "23",
    name: "LG 75-Inch NanoCell 8K TV",
    img: "https://www.lg.com/eastafrica/images/tvs/md07537767/gallery/TV-QNED-75-QNED95-A-Gallery-D01.jpg",
    category: "Electronics",
    price: 2499.99,
    rating: 4.7,
    description:
      "Experience stunning 8K visuals with this LG NanoCell 75-inch TV.",
    comments: [
      "The picture quality is breathtaking!",
      "Smart features are user-friendly.",
      "A premium TV with a premium price.",
    ],
  },
  {
    id: "24",
    name: "Cuisinart 12-Cup Coffee Maker",
    img: "https://pisces.bbystatic.com/image2/BestBuy_US/images/products/1626/1626072cv11d.jpg",
    category: "Appliances",
    price: 69.99,
    rating: 4.3,
    description:
      "Start your day with freshly brewed coffee using this Cuisinart coffee maker.",
    comments: [
      "Makes great coffee and easy to clean.",
      "Affordable and reliable.",
      "Some plastic parts feel flimsy.",
    ],
  },
  {
    id: "25",
    name: "North Face Waterproof Jacket",
    img: "https://m.media-amazon.com/images/I/61FppEa-9AL.jpg",
    category: "Clothing",
    price: 149.99,
    rating: 4.2,
    description:
      "Stay dry and stylish in wet weather with this North Face waterproof jacket.",
    comments: [
      "Keeps me dry in heavy rain.",
      "Comfortable to wear and good fit.",
      "Zippers could be more durable.",
    ],
  },
  {
    id: "26",
    name: "Breville Smart Oven Air Fryer",
    img: "https://assets.wsimgs.com/wsimgs/rk/images/dp/wcm/202342/0024/breville-joule-oven-air-fryer-pro-o.jpg",
    category: "Appliances",
    price: 299.99,
    rating: 4.8,
    description:
      "Cook delicious and healthy meals with the Breville Smart Oven Air Fryer.",
    comments: [
      "Versatile and cooks evenly.",
      "Air frying is a game-changer.",
      "A bit expensive, but worth it for the features.",
    ],
  },
  {
    id: "27",
    name: "Ray-Ban Aviator Sunglasses",
    img: "https://india.ray-ban.com/media/catalog/product/0/r/0rb30259196s262_1.jpg",
    category: "Clothing",
    price: 129.99,
    rating: 4.5,
    description: "Protect your eyes in style with Ray-Ban Aviator sunglasses.",
    comments: [
      "Classic and timeless design.",
      "High-quality lenses and comfortable fit.",
      "A bit on the pricey side, but worth it for the brand.",
    ],
  },
];
